...
Over 500 hours of rTSLA candles, 395 of them sat inside windows where the U.S. market was closed. 68.4% of those hours drifted more than 20 bps from the last official close. The median was 36 bps. The 90th percentile was 121.
...
**It names its own basis.** While the underlying can trade, only a fresh authenticated quote counts. While it can't, the last official close is the correct basis — and the desk says which tier it used. A gap against a closed market is drift, not a broken alignment: there is no live price for the token to disagree with.
...
Qwen writes the explanation, but only from evidence the server retrieved, and every claim has to cite a resolvable evidence ID. When nothing matches the timing it says NO_STRONG_CATALYST instead of inventing a cause. A refusal is a recorded outcome here, with its own row in the audit trail and a refusal rate — not a failure to hide.
