...
Not yet completed: a balanced matched-source benchmark and external user testing. The public rToken path, authenticated Stock+ quote path, question-aware Qwen workflow, citation gate, natural-language task, and first missing-reference benchmark slice have been verified in production. On 2026-09-14, the AAPL production run returned both the rToken and Stock+ prices and calculated a deterministic +14 bps comparison; the state remained `CAUTION` because the weekend underlying quote was stale. The authenticated Stock+ historical-candlestick route returned success but no rows for the probed non-trading window, so the original frozen benchmark was not overwritten. The official `bitget-signal` MCP was probed for stock/macro/news context but was not promoted into the live path because the required probes returned errors or empty data with roughly 20-second latency. Next: capture matched candles during an observable U.S. session and run tester sessions.
