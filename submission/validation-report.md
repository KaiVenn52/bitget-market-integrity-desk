...
|---|---:|---|
| Deterministic, decision, checkpoint, gate, study, routing and rate-limit unit tests | 235/235 passed, eleven suites | Includes Stock+ ISO timestamp and embedded-session parsing, stale-reference Gate refusal and unknown alignment, stateless MCP envelope, current calendar schema, non-cash-event abstention, and nine browser-local checkpoint regressions. `npm.cmd test` |
| Thesis Checkpoint | Local and production browser QA passed | A labelled opening snapshot could not be saved. A real production scan enabled saving; the trader's thesis and condition persisted through reload; only a later same-symbol live scan produced a before/after comparison. Local controlled QA also verified replacement and two-step removal. Checked at 1440px and 390px with no document overflow or console errors. Live QA exposed two false-diff risks: quote-age-only result text and a late historical study that populated the reference basis after a premature save. The code now counts check-state changes only and blocks saving/comparison until both research branches settle. The comparison does not adjudicate free-text claims or send alerts. |
...
| MCP pre-video incident (2026-09-26) | Current external outage; desk abstains | Repeated production scans received no usable MCP entries; the legacy handshake returned `503 Too many open sessions`. The current server advertised only stateless protocol `2026-07-28`; a direct new-protocol probe reached all four entries, but every data query returned a nested upstream `HTTP 503`. The updated client was deployed as `dpl_CwrXRbyrTnmC2aedkB6Z6swzBSLy`. A post-deployment scan requested the current `equity_calendar` and all four queries reported `HTTP 503`; the old `Unknown entry_id` was gone. This proves the fix is live, **not** that the upstream recovered. The proof strip reports the actual answered/requested count. |
| REST bypass and schema audit (2026-09-26) | REST did not bypass the outage; two schema mismatches fixed locally | The documented direct GET paths for quote, historical, dividends and calendar each returned HTTP 503; a valid crypto ticker and news request also returned 503. Current official docs specify millisecond Unix timestamps for history/dividends and disclosure-date fields for `equity_calendar`, not the former `report_date`/EPS schema. The client now sends typed filters and parses forecast versus actual disclosure dates without treating `period_ending` as a report date. A documented split/stock-dividend row cannot be counted as a cash dividend. These local changes are not evidence of upstream recovery. |
| Latest schema fix in production (2026-09-26) | Deployed; upstream remains unavailable | Commit `edf1886` deployed as `dpl_7983GQToaisw3CxzqwXF14yNq7U3`. A post-deployment `rAAPLUSDT` scan was live and returned 0/4 MCP entries; each current entry failed with `HTTP 503`, and corporate context stayed `UNVERIFIABLE`. The current code therefore does not misstate a successful official data integration. |
| MCP dispatch model | Serial, conservative | Four concurrent queries on the former session-based transport left three hanging until timeout. The current transport is stateless; serial dispatch remains until its concurrency is tested on a healthy server. |
...
| Corporate-action check | Dated cash-dividend result when the source answers | The earlier live sample reported one cash-dividend event dated 2026-09-09. The current documented entry can also return stock splits and stock dividends; those now cause abstention on adjustment rather than being mislabelled cash dividends. The historical candles have no adjustment field. |
...
| Local production-mode build | Passed | `npm.cmd run build` — Vite 8, 289 kB JS / 46 kB CSS; this does not prove deployment. |
