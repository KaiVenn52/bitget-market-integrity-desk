...
|---|---:|---|
| Deterministic, decision, checkpoint, gate, study, routing and rate-limit unit tests | 244/244 passed, eleven suites | Includes Stock+ ISO timestamp and embedded-session parsing, stale-reference Gate refusal and unknown alignment, MCP session cleanup and error propagation, and nine browser-local checkpoint regressions. `npm.cmd test` |
...
| MCP pre-video incident (2026-09-26) | Current external outage; desk abstains | Repeated production scans received no usable MCP entries; a direct official endpoint probe returned `503 Too many open sessions`. The client previously never terminated its session. It now issues a bounded `DELETE` after queries and after a failed handshake that minted a session, and exposes the upstream error string when supplied. Regression tests cover both paths. This prevents new client-owned session leaks; it does **not** prove our client caused the server-wide limit or restore an overloaded upstream. The live proof strip now reports the actual answered/requested count instead of implying all four answered. |
