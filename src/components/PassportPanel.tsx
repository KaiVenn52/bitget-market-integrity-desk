...
export function PassportPanel({ passport }: { passport: Passport }) {
  ...
  return <div className="passport-stack">
    <section className="passport-overview">
      ...
      {/* The perception layer is stated rather than implied: a judge should be able to
          see which official catalog entries answered, and which did not, without
          reading the network log. */}
      {passport.integration ? <div className="source-stack">
        <div className="source-head"><span className="section-kicker">Official Bitget sources</span><b>{passport.integration.server}{passport.integration.version ? ` v${passport.integration.version}` : ''}</b><em>{passport.integration.answered.length} of {passport.integration.requested.length} catalog entries answered</em></div>
        <ul>{passport.integration.requested.map((entry) => { const answered = passport.integration?.answered.includes(entry); return <li key={entry} className={answered ? 'source-ok' : 'source-miss'}><code>{entry}</code><span>{answered ? 'answered' : 'unavailable'}</span></li> })}</ul>
      </div> : null}
    </section>
