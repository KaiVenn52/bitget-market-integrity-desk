import benchmarkCases from '../../benchmark/cases.json'
...
export function ReplayLab() {
  ...
  return <main className="content-view replay-view">
    ...
    <section className="panel replay-table">
      <table><thead><tr><th>Case</th><th>Instrument</th><th>Evidence cutoff</th><th>Reference</th><th>Observed result</th></tr></thead><tbody>{benchmarkResults.rows.map((item, index) => <tr key={item.id}><td data-label="Case">B{String(index + 1).padStart(2, '0')}</td><td data-label="Instrument">{item.symbol}</td><td data-label="Evidence cutoff">{new Date(item.cutoff).toISOString().slice(11, 16)} UTC</td><td data-label="Reference" className="caution-text">MISSING</td><td data-label="Observed result">{item.predictedState}</td></tr>)}</tbody></table>
