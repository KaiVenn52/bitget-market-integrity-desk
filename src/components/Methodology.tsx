...
export function Methodology() {
  return <main className="content-view methodology-view">
    <div className="view-heading">
      <div>
        <h1>Methodology</h1>
        <p>AI analyzes the move. The integrity layer proves which parts of that explanation are actually supported — and refuses the ones that are not.</p>
      </div>
    </div>
    ...
    <section className="method-rail">
      ...
      <div><span>03</span><h2>Explain</h2><p>Rank only headlines that name the instrument, by publication time against the move start. A headline published after the move is labelled timing-inconsistent, never a cause. The model may narrate a verdict; it may not invent one, and every claim it makes must cite an evidence id the server supplied.</p></div>
    ...
    <section className="panel reference-tiers">
      ...
      <div className="tier-grid">
        ...
        <div>
          <span className="mode-badge mode-rules">LAST OFFICIAL CLOSE</span>
          <p>While the main session is not running, the price the underlying last traded at <em>is</em> the correct basis. This tier is retrievable without an authenticated feed, which is why the desk's central case — an overnight drift nothing can confirm — does not depend on a credential. A gap against this tier is drift, not an alignment break: there is no live underlying price for the token to disagree with.</p>
        </div>
      </div>
      <p className="study-context">Both tiers are labelled in the interface and repeated in every verdict that rests on them, so a reader can always tell a verified basis from an unverified one.</p>
    </section>
